...
import { getDownloadUrl, getOptimizedPreviewUrl } from '../../scripts/polaris.js';
import { addMetadataFields, formatAssetMetadata } from '../../scripts/metadata.js';
 ...
 */
function addDownloadHandlers(cardElement) {
  cardElement.querySelectorAll('.actions-download').forEach((el) => {
    el.addEventListener('click', (e) => {
      e.preventDefault();
      downloadImage(e.target.href, e.target.dataset.name);
    });
  });
}
...
export async function selectNextAsset() {
  if (currentlySelectedAssetCard) {
    ...
    if (nextAssetCard) {
      ...
      openAssetDetails(nextAssetCard.dataset.assetId);
    }
  }
}
...
export async function selectPreviousAsset() {
  if (currentlySelectedAssetCard) {
    ...
    if (previousAssetCard) {
      ...
      openAssetDetails(previousAssetCard.dataset.assetId);
    }
  }
}
...
function addMetadataField(metadataElement, metadataInfo) {
  ...
  const labelPar = document.createElement('p');
  labelPar.classList.add('label');
  labelPar.textContent = label;
  item.appendChild(labelPar);
  const valuePar = document.createElement('p');
  valuePar.classList.add('value');
  if (isElement(value)) {
    valuePar.appendChild(formatAssetMetadata(name, value, dataType));
    metadataElement.appendChild(item);
  } else {
    if (value === undefined || value === null) {
      valuePar.textContent = '--';
    } else {
      const formattedValue = formatAssetMetadata(name, value, dataType);
      if (!formattedValue || formattedValue.trim() === '') {
        valuePar.textContent = '--';
      } else {
        valuePar.textContent = formattedValue;
        valuePar.title = formattedValue;
      }
    }
    item.appendChild(valuePar);
  }
  metadataElement.appendChild(item);
}
...
function createCardElement(hit) {
  ...
  <div class="actions">
      ...
      <a class="actions-share" href="javascript:void()"><span class="icon icon-share"></span>Share</a>
  </div>`;
  ...
  addMetadataFields(
    ...
    hit,
    (metadataInfo) => {
      addMetadataField(metadataElem, metadataInfo);
    },
  );
  ...
  const emptySpaces = (numOfRows * numOfColumns) - metadataElem.children.length;

  function markTruncatedFields() {
    const metadataNodes = metadataElem.querySelectorAll('.metadata-field');
    ...
    const truncatedFields = metadataElem.querySelectorAll('.metadata-field.truncated');
    if (emptySpaces === 1) {
  ...
  if (emptySpaces > 0) {
    ...
    resizeObserver.observe(metadataElem);
  }

  const actionsDownloadA = card.querySelector('.actions-download');
  actionsDownloadA.download = true;
  actionsDownloadA.dataset.name = repoName;
  actionsDownloadA.href = getDownloadUrl(assetId, repoName);

  decorateIcons(card);
  handleImageFailures(card);
  addDownloadHandlers(card);
